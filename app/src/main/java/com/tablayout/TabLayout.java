...
public class TabLayout extends HorizontalScrollView {
    ...
    public TabLayout(@NonNull Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        ...
        scrollableTabMinWidth = res.getDimensionPixelSize(com.google.android.material.R.dimen.design_tab_scrollable_min_width);
    ...
    void populateFromPagerAdapter() {
        ...
        if (pagerAdapter != null) {
            ...
            for (int i = 0; i < adapterCount; i++) {
                addTab(newTab().setText(mIsPageAdapterTitleVisible ? pagerAdapter.getPageTitle(i) : ""), false);
            }
    ...
    private void setTitleStatus(final Tab tab) {
        final TabLayout tabLayout = this;
        //还原文字效果
        if (tab == null) {
            return;
        }
        for (int i = 0; i < tabs.size(); i++) {
            if (tabs.get(i).customView != null && tab != tabs.get(i)
                    && (tabLayout.tabs.get(i).customView.getTag() instanceof PageTransformerListener)) {
                PageTransformerListener target = (PageTransformerListener) tabs.get(i).customView.getTag();
                target.onEnter(i, 0, true);
            }
        }
        if (tab.customView != null && tab.customView.getTag() instanceof PageTransformerListener) {
            PageTransformerListener target = (PageTransformerListener) tab.customView.getTag();
            target.onEnter(getSelectedTabPosition(), 1, true);
            int newPosition = tab.getPosition();
            if (newPosition != Tab.INVALID_POSITION) {
                setSelectedTabView(newPosition);
            }
        }
        ViewCompat.postInvalidateOnAnimation(tabLayout.slidingTabIndicator);
    }
    ...
    public final class TabView extends LinearLayout {
            ...
            this.iconView =
                    (ImageView)
                            LayoutInflater.from(getContext())
                                    .inflate(com.google.android.material.R.layout.design_layout_tab_icon, iconViewParent, false);
        ...
        private void inflateAndAddDefaultTextView() {
            ...
            this.textView =
                    (TextView)
                            LayoutInflater.from(getContext())
                                    .inflate(com.google.android.material.R.layout.design_layout_tab_text, textViewParent, false);
    ...
    private class SlidingTabIndicator extends LinearLayout {
        ...
        public int mSelectedIndicatorWidth = -1;
        private float left;
        private float right;
        public boolean mIsDragged;
        public boolean mIsIndicatorWidthWrapContent;
        ...
        protected void onLayout(boolean changed, int l, int t, int r, int b) {
            ...
            if (viewPager != null && viewPager.getCurrentItem() >= 0) {
                setScrollPosition(viewPager.getCurrentItem(), 0f, true);
                updateBgPosition();
                setTitleStatus(tabs.get(viewPager.getCurrentItem()));
            }
        ...
        private void updateBgPosition() {
            ...
            if (selectedTitle != null && selectedTitle.getWidth() > 0) {
                ...
                if (mIsIndicatorWidthWrapContent){
                    right = left+getContentIndicatorWidth(selectedPosition);
                }
                mIsDragged = false;
        ...
        void animateIndicatorToPosition(final int position, int duration) {
            ...
            if (mIsIndicatorWidthWrapContent){
                ...
                targetRight = getChildAt(position).getLeft() + getContentIndicatorWidth(position);
            }
        ...
        public void draw(@NonNull Canvas canvas) {
            ...
            if (indicatorLeft >= 0 && indicatorRight > indicatorLeft) {
                mSelectedIndicatorWidth = getContentIndicatorWidth(getSelectedTabPosition());
              ...
              /*  if (getSelectedTabPosition() >= 0) {
                    View current = getChildAt(getSelectedTabPosition());
                    Tab tab = tabs.get(getSelectedTabPosition());
                    View currentCustomView = tab.customView;
                    if (currentCustomView == null) {
                        currentCustomView = current;
                    }
                    mRect.top = current.getTop() + ((getHeight() - currentCustomView.getHeight()) * 1f / 2);
                    mRect.bottom = mRect.top + currentCustomView.getHeight();
                    if (!mRoundRadiusSet) {
                ...
                if (mSelectedIndicatorWidth > 0 && getSelectedTabPosition() >= 0) {
                    if (mIsDragged) {
                        indicatorLeft = (int) left;
                        indicatorRight = (int) right;
                    } else {
                        left = indicatorLeft;
                    ...
                    mRect.left = left;
                    mRect.right = right;
                    selectedIndicator.setBounds((int) left, indicatorTop, (int) right, indicatorBottom);
                    canvas.drawRoundRect(mRect, mRoundRadius, mRoundRadius, mPaint);
                    mIsDefaultIndicatorWidth = false;
                } else {
                    mRect.left = indicatorLeft;
                    mRect.right = indicatorRight;
        ...
        private int getContentIndicatorWidth(int index) {
            ...
            if (mIsIndicatorWidthWrapContent && getChildAt(index) != null) {
                ...
                if (childAt instanceof TabView) {
                    ...
                    if (tabView.iconView != null && tabView.iconView.isShown()) {
                        mWidthList.add(tabView.iconView.getMeasuredWidth());
                    }
                    if (tabView.textView != null && tabView.textView.isShown()) {
                        mWidthList.add(tabView.textView.getMeasuredWidth());
                    }
                    if (tabView.customIconView != null && tabView.customIconView.isShown()) {
                        mWidthList.add(tabView.customIconView.getMeasuredWidth());
                    }
                    if (tabView.customTextView != null && tabView.customTextView.isShown()) {
                        mWidthList.add(tabView.customTextView.getMeasuredWidth());
                    }
                    if (!mWidthList.isEmpty()) {
                        Collections.sort(mWidthList);
                        width = mWidthList.get(mWidthList.size()-1);
                    } else {
                        width = childAt.getMeasuredWidth() - tabPaddingStart - tabPaddingEnd;
                    }
                } else {
                    width = childAt.getMeasuredWidth() - tabPaddingStart - tabPaddingEnd;
                }
    ...
    public static class TabLayoutOnPageChangeListener implements ViewPager.OnPageChangeListener {
        ...
        public void onPageScrolled(
            ...
            if (tabLayout != null) {
                ...
                tabLayout.setScrollPosition(position, positionOffset, false, updateIndicator);

                if (scrollState == SCROLL_STATE_SETTLING && previousScrollState == SCROLL_STATE_IDLE) {
                    tabLayout.slidingTabIndicator.mIsDragged = false;
                } else {
                    tabLayout.slidingTabIndicator.mIsDragged = true;
                }

                int currentPos = tabLayout.slidingTabIndicator.selectedPosition;
                View current = tabLayout.slidingTabIndicator.getChildAt(currentPos);
                if (!(current instanceof TabView)) {
                    return;
                }
                switchAnimation(position, positionOffset, tabLayout, currentPos, current);
            }
        }

        private void switchAnimation(int position,float positionOffset, TabLayout tabLayout,
                                     int currentPos, View current) {
            if (tabLayout.mNeedSwitchAnimation && tabLayout.slidingTabIndicator.mIsDragged) {
                if (currentPos + 1 < tabLayout.slidingTabIndicator.getChildCount()) {
                    //设置指示器效果
                    float offset = (current.getMeasuredWidth()
                            - tabLayout.slidingTabIndicator.getContentIndicatorWidth(currentPos)) / 2.0f;
                    float leftX = current.getLeft() + offset;
                    float rightX = leftX + tabLayout.slidingTabIndicator.getContentIndicatorWidth(currentPos);
                    float nextOffset = (tabLayout.slidingTabIndicator.getChildAt(position).getMeasuredWidth()
                            - tabLayout.slidingTabIndicator.getContentIndicatorWidth(position)) / 2.0f;
                    int direction = 1;
                    if (position - tabLayout.slidingTabIndicator.selectedPosition >= 0) {
                        direction = 1;
                    } else {
                        direction = -1;
                    }
                    float nextLeftX = current.getLeft() + direction * tabLayout.slidingTabIndicator.getChildAt(position).getWidth() + nextOffset;
                    float nextRightX = nextLeftX + tabLayout.slidingTabIndicator.getContentIndicatorWidth(tabLayout.slidingTabIndicator.selectedPosition + 1);
                    tabLayout.slidingTabIndicator.left = leftX + (nextLeftX - leftX) * mStartInterpolator.getInterpolation(positionOffset);
                    tabLayout.slidingTabIndicator.right = rightX + (nextRightX - rightX) * mEndInterpolator.getInterpolation(positionOffset);
                    ViewCompat.postInvalidateOnAnimation(tabLayout.slidingTabIndicator);
                }
            }
        ...
        private void dispatchOnEnter(int index, float enterPercent, boolean leftToRight, boolean force) {
            if (index == mCurrentIndex || scrollState == SCROLL_STATE_DRAGGING || force) {
                final TabLayout tabLayout = tabLayoutRef.get();
                if (tabLayout != null) {
                    Tab tab = tabLayout.tabs.get(index);
                    View customView = tab.customView;
                    if (customView == null) {
                        return;
                    }
                    if (customView.getTag() instanceof PageTransformerListener) {
                        PageTransformerListener target = (PageTransformerListener) customView.getTag();
                        target.onEnter(index, enterPercent, leftToRight);
                    }
                }
            }
        }
        ...
        private void dispatchOnLeave(int index, float leavePercent, boolean leftToRight, boolean force) {
            if (index == mLastIndex || scrollState == SCROLL_STATE_DRAGGING  || force) {
                final TabLayout tabLayout = tabLayoutRef.get();
                if (tabLayout != null) {
                    Tab tab = tabLayout.tabs.get(index);
                    View customView = tab.customView;
                    ...
                    if (customView.getTag() instanceof PageTransformerListener) {
                        PageTransformerListener target = (PageTransformerListener) customView.getTag();
                        target.onLeave(index, leavePercent, leftToRight);
                    }
                }
            }
        }
        ...
        public void onPageSelected(final int position) {
            final TabLayout tabLayout = tabLayoutRef.get();
            if (tabLayout != null){
                mLastIndex = tabLayout.getSelectedTabPosition();
    ...
    private int mFillColor;  //指示器颜色
    private float mRoundRadius; //指示器背景圆角
    ...
    public void setDefaultIndicatorWidth(boolean defaultIndicatorWidth) {
        mIsDefaultIndicatorWidth = defaultIndicatorWidth;
    }

    public void setSelectedIndicatorWidth(int selectedIndicatorWidth) {
        if (slidingTabIndicator.mSelectedIndicatorWidth != selectedIndicatorWidth) {
            slidingTabIndicator.mSelectedIndicatorWidth = selectedIndicatorWidth;
            ViewCompat.postInvalidateOnAnimation(this);
        }
    }

    public int getSelectedIndicatorWidth() {
        return slidingTabIndicator.mSelectedIndicatorWidth;
    }

    public boolean isIndicatorWidthWrapContent() {
        return slidingTabIndicator.mIsIndicatorWidthWrapContent;
    }

    public void setIndicatorWidthWrapContent(boolean indicatorWidthWrapContent) {
        if (slidingTabIndicator.mIsIndicatorWidthWrapContent != indicatorWidthWrapContent) {
            slidingTabIndicator.mIsIndicatorWidthWrapContent = indicatorWidthWrapContent;
            ViewCompat.postInvalidateOnAnimation(this);
        }
    }

    private boolean mIsPageAdapterTitleVisible = true;

    public void setPageTitleVisible(boolean isVisible) {
        mIsPageAdapterTitleVisible = isVisible;
    }
